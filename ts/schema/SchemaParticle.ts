...
export abstract class SchemaParticle {
    ...
    abstract matchOnce(children: string[], pos: number, nsMap?: Map<string, string>, childNamespaces?: string[]): number[];

    matchRepeated(children: string[], startPos: number, nsMap?: Map<string, string>, childNamespaces?: string[]): number[] {
        ...
        for (let count: number = 1; count <= max; count++) {
            ...
            for (const pos of currentPositions) {
                const matched: number[] = this.matchOnce(children, pos, nsMap, childNamespaces);
