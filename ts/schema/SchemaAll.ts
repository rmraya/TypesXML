...
export class SchemaAll extends SchemaParticle {
    ...
    private matchRemaining(children: string[], pos: number, remaining: number[], nsMap?: Map<string, string>, childNamespaces?: string[]): number[] {
        ...
        for (let i: number = 0; i < remaining.length; i++) {
            ...
            for (const nextPos of matched) {
                if (nextPos > pos) {
                    ...
                    const further: number[] = this.matchRemaining(children, nextPos, newRemaining, nsMap, childNamespaces);
